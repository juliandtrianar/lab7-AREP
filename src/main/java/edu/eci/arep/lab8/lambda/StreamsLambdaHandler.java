...
public class StreamsLambdaHandler implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
    ...
    public APIGatewayProxyResponseEvent handleRequest(APIGatewayProxyRequestEvent event, Context context) {
        try {
            ...
            if ("GET".equalsIgnoreCase(httpMethod)) {
                ...
                String responseBody = objectMapper.writeValueAsString(savedPost);
                return createResponse(201, responseBody, "application/json");
            } else {
                return createResponse(405, "Method Not Allowed", "text/plain");
            }
        } catch (Exception ex) {
            context.getLogger().log("StreamsLambdaHandler Error: " + ex.getMessage());
